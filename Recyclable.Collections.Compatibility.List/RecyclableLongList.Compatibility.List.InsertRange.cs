...
#pragma warning disable IDE1006 // This intentional so that it shows at the end of IntelliSense list
	public static class zzzRecyclableLongListCompatibilityListInsertRange
#pragma warning restore IDE1006 // Naming Styles
	{
		private static void InsertRangeEnumerated<T>(this RecyclableLongList<T> list, int index, IEnumerable<T> items, int growByCount)
		{
		...
		private static void InsertRangeWithKnownCount<T>(this RecyclableLongList<T> list, int index, IEnumerable<T> items, int currentItemsCount, int requiredAdditionalCapacity)
		{
			...
			//// TODO: Count should be requiredAdditionalCapacity, we need to move as many items as many we're inserting
		...
		public static void InsertRange<T>(this RecyclableLongList<T> list, int index, in Array items)
		{
			...
			//T[] memoryBlock = list._memoryBlock;
			//// TODO: Count should be items.Count, we need to move as many items as many we're inserting
		...
		public static void InsertRange<T>(this RecyclableLongList<T> list, int index, IEnumerable items, int growByCount = RecyclableDefaults.MinPooledArrayLength)
		{
			...
			//	for (i = 0; i < available; i++)
			//	{
			//		memorySpan[index++] = (T)enumerator.Current;
			//		if (!enumerator.MoveNext())
			//		{
			//			added++;
			//			break;
		...
		public static void InsertRange<T>(this RecyclableLongList<T> list, int index, IReadOnlyList<T> items)
		{
			...
			//Array.Copy(list._memoryBlock, index, list._memoryBlock, index + sourceItemsCount, Math.Min(list.Count - index, sourceItemsCount));
			//Span<T> memorySpan = new(list._memoryBlock, index, sourceItemsCount);
